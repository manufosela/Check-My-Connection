...
CheMyCo = (function (){
    ...
    CheMyCo.prototype.init = function ( url ){
        var _this = this;
        if ( typeof url != "undefined" ) { this.url = url; } 
        if ( this.url != "") { setInterval( function(){ _this.checkConn(); }, _this.checkIntrvl); }
    }

    CheMyCo.prototype.checkConn = function() {
        var xhr, _this = this;  
            ...
            if (xhr.status == 200) {
                if ( _this.url == _this.defaultUrl ) {
                    ...
                    if ( !!~xhr.responseText.toLowerCase().indexOf("error") ) {
                        _this._L("#chMsg").innerHTML = _this.notdbhtml;
        ...
        xhr.open("GET", this.url+"?notcache="+Math.random(), true);
        xhr.send();
    }
