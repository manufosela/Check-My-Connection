/* by @manufosela - chemyco.js - Check My Connection - 20131113 - v1.0 */
(function () {
    var _L = function (a) {
        if ( !! ~a.indexOf("#")) {
    ...
    //_L("#" + idCh).style.display = "block";
      ...
      xhr.onreadystatechange = function(e) {
        ...
        if (xhr.status == 200) {
        	if ( url == "chemyco.js") {
    			_L("#" + idCh + "k").style.display = "none";
	  			_L("#" + idCh).style.display = "none";
    	  	} else {
	       		if ( !!~xhr.responseText.toLowerCase().indexOf("error") ) {
	       			_L("#chMsg").innerHTML = notdbhtml;
    		  		_L("#" + idCh + "k").style.display = "block";
    		  		_L("#" + idCh).style.display = "block";
    	  		} else {
          			_L("#" + idCh + "k").style.display = "none";
        	...
        	_L("#chMsg").innerHTML = notconhtml;
          	_L("#" + idCh + "k").style.display = "block";
    	  	_L("#" + idCh).style.display = "block";
        }
      };
      xhr.open("GET", url+"?notcache="+Math.random(), true);
      xhr.send();
    };  
    var ready = function () {
        function i() {
        ...
        var r = {
            ...
            ready: function (t) {
                if (t === true && !--r.readyWait || t !== true && !r.isReady) {
                    if (!document.body) {
                        return setTimeout(r.ready, 1)
                    }
                    r.isReady = true;
                    if (t !== true && --r.readyWait > 0) {
                        return
                    }
                    e.resolveWith(document, [r])
                }
            },
            bindReady: function () {
                if (e) {
                ...
                var e = [],
                    t, n, i, s = {
                            ...
                            if (!i) {
                                ...
                                if (l) {
                                    s.resolveWith(l[0], l[1])
                                }
                            }
                            return this
                        },
                        resolveWith: function (r, s) {
                            if (!i && !t && !n) {
                                s = s || [];
                                n = 1;
                                try {
            ...
            type: function (e) {
                return e == null ? String(e) : n[Object.prototype.toString.call(e)] || "object"
            }
        };
        if (document.addEventListener) {
            t = function () {
    ...
    ready(function () {
      if ( url != "") { setInterval(function(){checkcondb(url);},checkcondbIntrvl); }
    });
})();